...
public class ATM extends Safe {
    ...
    private String bank;
    private OnMoneyListener listener; // переменная интерфейяа
    ...
    private int[] setPaper(int count){
        ...
        else{  // списываем деньги, так как данную сумму можно снять
            setPaper100(getPaper100() - money[0]);
            setPaper50(getPaper50() - money[1]);
            setPaper20(getPaper20() - money[2]);
        }
    ...
    public void put(int password, int p20, int p50, int p100){
        if(password == pass)
        {
